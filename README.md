...
本プロジェクトはClaude（Anthropic）を活用して開発しました。                          
設計・実装・デバッグの各フェーズでAIとの対話を通じて進め、コードレビューや技術的な意思決定にも活用しています。  

## 設計上の意思決定

開発中に直面したトレードオフと、その判断の理由をまとめます。

### Google Calendar APIをRoute Handlerに移動した

当初はブラウザから直接Google Calendar APIを叩いていましたが、その場合`provider_token`（Googleのアクセストークン）がブラウザに渡ってしまいます。他のユーザーに使ってもらうことを想定し、セキュリティ上の問題があると判断してサーバー側のRoute Handlerで処理するよう移行しました。

### メール生成はクライアント側のままにした

`/api/mail/generate` Route Handlerを実装しましたが、`page.tsx`からの呼び出しはあえて`useMemo`のままにしています。空き時間帯のチェックを切り替えるたびにメール文がリアルタイムで更新されるUXを優先し、ネットワーク通信を挟むトレードオフを避けました。

### ログインなし対応を維持した

「誰でも気軽にすぐ使える」というコンセプトを重視し、Googleログインを必須にしませんでした。`/api/interviews/judge`は認証不要とし、カレンダーイベントをリクエストボディで受け取る設計にすることで、未ログインユーザーは空のイベント配列を渡して重複チェックなしで利用できます。

### データの永続化にハイブリッド戦略を採用した

ログイン済みユーザーはSupabase DB（複数デバイス対応）、未ログインユーザーはlocalStorageと、状態に応じて永続化先を切り替えています。また`useLocalStorage`にハイドレーションフラグ（`isHydrated`）を導入し、マウント直後に初期値でlocalStorageを上書きしてしまうバグを修正しました。

### 複数企業管理はシンプルなタブUIに絞った

企業ごとに別ページへ遷移する設計も検討しましたが、「気軽に使える」コンセプトと一致しないと判断しました。タブを切り替えるだけで企業間を移動でき、ルーティングを増やさずに済むシンプルな実装を選びました。
